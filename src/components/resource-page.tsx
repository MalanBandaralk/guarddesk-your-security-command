...
export function ResourcePage({ kind }: { kind: PageKind }) {
  ...
  const rows: Array<Record<string, string | number>> = buildRows(kind, data).filter((row) => Object.values(row).join(" ").toLowerCase().includes(query.toLowerCase()));
...
function buildRows(kind: PageKind, data: ReturnType<typeof useWorkspaceData>["data"]): Array<Record<string, string | number>> {
